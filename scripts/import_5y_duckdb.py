#!/usr/bin/env python3
"""Test DuckDB MySQL ATTACH and import 5-year data"""
import duckdb
import time
import os
...
def main():
    ...
    # Check stock_daily date range
    r = conn.execute("SELECT MIN(trade_date), MAX(trade_date), COUNT(*) FROM mysql_db.stock_daily").fetchone()
    print(f'stock_daily range: {r[0]} to {r[1]}, rows: {r[2]:,}')
