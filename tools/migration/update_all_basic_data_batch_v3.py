    ...
    for i in tqdm(range(0, len(stock_codes), batch_size), desc="Updating basic data"):
        ...
        try:
            ...
            if df_basic is not None and not df_basic.empty:
                ...
                # 每批打印一次进度
                print(f"\n✓ 批次 {i}-{i+len(batch_codes)}: 更新了 {len(df_basic)} 只股票")
                
            else:
                total_skipped += len(batch_codes)
                print(f"\n⚠️  批次 {i}-{i+len(batch_codes)}: 没有获取到数据（累计跳过 {total_skipped} 只）")
                
            # 速率限制：每批之间等待0.5秒
            time.sleep(0.5)
                
        except Exception as e:
            print(f"\n✗ Error updating batch {i}-{i+len(batch_codes)}: {e}")
