...
def update_fundamental_data_batch():
    ...
    # 获取最新的交易日
    df_trade = pro.trade_cal(exchange='SSE', start_date='20250101', end_date='20250206')
    if df_trade is not None and not df_trade.empty:
        latest_trade_date = df_trade[df_trade['is_open'] == 1]['cal_date'].max()
        print(f"最新交易日: {latest_trade_date}")
    else:
        latest_trade_date = '20250205'
        print(f"使用默认日期: {latest_trade_date}")
    ...
    for i in tqdm(range(0, len(stock_codes), batch_size), desc="Updating basic data"):
        ...
        try:
            ...
            if df_basic is not None and not df_basic.empty:
                ...
                # 每批打印一次进度
                print(f"✓ 已更新 {total_updated}/{len(stock_codes)} 只股票")
                
            else:
                print(f"\n⚠️  批次 {i}-{i+batch_size} 没有获取到数据")
